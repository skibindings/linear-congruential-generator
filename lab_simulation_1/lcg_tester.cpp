...
double LCGTester::test_lcg(FishmanLCG& lcg, unsigned int d, bool parallel) {
    d_ = d;
    ...
    // отдельно для k потоков будем собирать статистики а потом суммировать
    unsigned int** frequency_v = new unsigned int*[kStreams];

    // задаем теоретические вероятности
    for (unsigned int i = 0; i < d_; i++) {
        frequency_p[i] = 1.0 / (long double)d_;
    }
    // инициализируем статистики
    for (int j = 0; j < kStreams; j++) {
        frequency_v[j] = new unsigned int[d_];
        for (unsigned int i = 0; i < d_; i++) {
            frequency_v[j][i] = 0;
        }
    }

    // то же самое для криетрия серий
    long double* serial_p = new long double[d_*d_];
    unsigned int** serial_v = new unsigned int*[kStreams];
    for (unsigned int i = 0; i < d_*d_; i++) {
        serial_p[i] = 1.0 / (long double) (d_*d_);
    }
    for (int j = 0; j < kStreams; j++) {
        serial_v[j] = new unsigned int[d_*d_];
        for (unsigned int i = 0; i < d_*d_; i++) {
            serial_v[j][i] = 0;
        }
    }

    // то же самое для криетрия купонов
    long double* coupon_p = new long double[max_coupon_size+1];
    unsigned int** coupon_v = new unsigned int*[kStreams];
    ...
    for (int j = 0; j < kStreams; j++) {
        coupon_v[j] = new unsigned int[max_coupon_size + 1];
        for (unsigned int i = 0; i < max_coupon_size + 1; i++) {
            coupon_v[j][i] = 0;
        }
    }

    // то же самое для криетрия покера
    long double* poker_p = new long double[5];
    unsigned int** poker_v = new unsigned int*[kStreams]; 
    for (unsigned int i = 0; i < max_coupon_size; i++) {
        poker_p[i] = calculate_poker_probability(i + 1);
    }
    for (int j = 0; j < kStreams; j++) {
        poker_v[j] = new unsigned int[5]; // хранит количество комбинаций с 1, 2, 3, 4, 5 различными числами
    ...
    if (parallel) { // считаем статисктики с распараллеливанием 
        #pragma omp parallel for 
        for (int step = 0; step < kStreams; step++) {
            collect_combinations(lcg, seeds_[step], frequency_v[step], serial_v[step], coupon_v[step], poker_v[step]);
        }
    }
    else { // считаем статисктики без распараллеливания 
        for (int step = 0; step < kStreams; step++) {
            collect_combinations(lcg, seeds_[step], frequency_v[step], serial_v[step], coupon_v[step], poker_v[step]);
        }
    }
    ...
    if(parallel)
        cout << "criteria calculations with omp duration : " << duration << endl;
    else 
        cout << "criteria calculations unparallel : " << duration << endl;

    // суммируем статистику из разных потоков
    unsigned int* final_frequency_v = new unsigned int[d_];
    for (int i = 0; i < d_; i++) {
        final_frequency_v[i] = frequency_v[0][i]; // для начала присвоим 
    }
    for (int j = 1; j < kStreams; j++) {
        for (int i = 0; i < d_; i++) {
            final_frequency_v[i] += frequency_v[j][i]; // теперь суммируем
        }
    }
    
    unsigned int* final_serial_v = new unsigned int[d_*d_];
    for (int i = 0; i < d_*d_; i++) {
        final_serial_v[i] = serial_v[0][i]; // для начала присвоим 
    }
    for (int j = 1; j < kStreams; j++) {
        for (int i = 0; i < d_*d_; i++) {
            final_serial_v[i] += serial_v[j][i]; // теперь суммируем
        }
    }

    unsigned int* final_coupon_v = new unsigned int[max_coupon_size + 1];
    for (int i = 0; i < max_coupon_size + 1; i++) {
        final_coupon_v[i] = coupon_v[0][i]; // для начала присвоим 
    }
    for (int j = 1; j < kStreams; j++) {
        for (int i = 0; i < max_coupon_size + 1; i++) {
            final_coupon_v[i] += coupon_v[j][i]; // теперь суммируем
        }
    }

    unsigned int* final_poker_v = new unsigned int[5];
    for (int i = 0; i < 5; i++) {
        final_poker_v[i] = poker_v[0][i]; // для начала присвоим 
    }
    for (int j = 1; j < kStreams; j++) {
        for (int i = 0; i < 5; i++) {
            final_poker_v[i] += poker_v[j][i]; // теперь суммируем
        }
    }

    frequency_score_ = calculate_score(d_, kSequenceSize, final_frequency_v, frequency_p);
    //cout << "frequency score = " << frequency_score_ << endl;

    serial_score_ = calculate_score(d_*d_, kSequenceSize / 2, final_serial_v, serial_p);
    //cout << "serial score = " << serial_score_ << endl;

    coupon_score_ = calculate_score(max_coupon_size + 1, kSequenceSize * (((long double)d_ - 1.0) / (long double)d_), final_coupon_v, coupon_p);
    cout << "coupon score = " << coupon_score_ << endl;

    poker_score_ = calculate_score(5, kSequenceSize / 5, final_poker_v, poker_p);
