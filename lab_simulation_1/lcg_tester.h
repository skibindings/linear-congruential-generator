...
public:
    ...
    double test_lcg(FishmanLCG& lcg, unsigned int d, bool parallel); // возращает длительность вычисления критериев
...
private:
    ...
    const unsigned int kSequenceSize{ (unsigned int) pow(2,27) }; // размер последовательности, которую будем исследовать
